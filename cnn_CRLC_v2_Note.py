...
tplt1 = "{0:^30}\t{1:^10}\t{2:^10}\t{3:^10}\t{4:^10}"  # 用于格式化输出
tplt = "{0:^30}\t{1:^10}\t{2:^10}"  # 用于格式化输出
...
config.gpu_options.per_process_gpu_memory_fraction = 1  # GPU内存使用最大比例，这里设置为1，就会限制共享内存的使用，所以可以注释
...
def init(sliceType, QP):
    ...
    # global mCRLC
    # QP = QP / 4 # aom传过来的是base_qpindex是0-255的，而参数配置的QP是0-64的，所以要除4
    # if QP ==32:
...
def test_all_ckpt(modelPath):
    ...
    for ckpt in ckpt_files:
        ...
        mCRLC = Predict(CRLC, os.path.join(modelPath, ckpt))  # mCRLC是全局变量，所以参数已经直接传进predict了
        sum_img_psnr = 0
        ...
        for i in img_index:
            ...
            sum_img_psnr = cur_img_psnr + sum_img_psnr  # 对图片psnr求和
