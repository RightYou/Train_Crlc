...
tf.logging.set_verbosity(tf.logging.INFO)  # 默认情况下，tensorflow在WARN的日志记录级别进行配置，但是在跟踪模型训练时，需要将级别调整为INFO
...
if __name__ == '__main__':
    ...
    with tf.Session(config=config) as sess:
        ...
        for epoch in range(last_epoch, last_epoch + MAX_EPOCH):
            ...
            tf.logging.info(
                "Epoch: [%4d/%4d]  time: %4.4f\t"
                "loss: %.8f\t"
                "lr: %.16f\t"
                "total_get_data_time: %8f\t"
                "total_network_time: %8f"
