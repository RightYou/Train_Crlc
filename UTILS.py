...
class Reader(threading.Thread):
    ...
    def run(self):
        ...
        # qp=int(self.file_name[0].split("\\")[-2].split("p")[1])
        # print(self.file_name[0],qp)
        in_ = []
        gt_ = []
        for j in range(BATCH_SIZE // 8):
            input_imgY, gt_imgY = crop(input_image, gt_image, PATCH_SIZE[0], PATCH_SIZE[1], "ndarray")
            # print(np.shape(input_imgY))
            # qp_img=np.ones([PATCH_SIZE[0],PATCH_SIZE[1]],dtype='uint8')*qp*4
            # input_imgY=np.dstack((input_imgY,qp_img))
            # print(np.shape(input_imgY))
...
def load_file_list(directory):
    ...
    # return sorted(list,key=lambda i:int(re.match("(\d+)_(\d+)[xX](\d+).yuv").group(0)))
    # print(len(list))
...
def get_list(low_path, hig_path):
    low_list = load_file_list(low_path)
    train_list = []
    for i in low_list:
        basename = os.path.basename(i)
        if 80 <= int(basename.split("_")[0].split("-")[1]) <= 120:
            basedir = i.split("\\")[-2]
            # basename=basename.split("-")[0]+basename.split("_")[1:]
            basename = re.sub("-(\d+)_", "_", basename)
            hi = os.path.join(hig_path, basedir, basename)
            train_list.append([i, hi])
    return train_list
        ...
        for j in range(bow):
            if i != boh - 1 and j != bow - 1:
                if i == 0 and j == 0:
                    ...
                    rec[block_size * i:block_size * (i + 1), block_size * (j):block_size * (j + 1)] = np.mat(
                        sub_imgs[i * bow + j])[
                                                                                                      pading_size:block_size + pading_size,
                                                                                                      pading_size:block_size + pading_size]
            elif i == boh - 1 and j != bow - 1:
