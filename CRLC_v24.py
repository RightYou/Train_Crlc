...
def conv2d(tensor, shape, name):
    with tf.variable_scope(name, reuse=tf.AUTO_REUSE):
        ...
        conv = tf.nn.conv2d(tensor, conv_w, strides=[1, 1, 1, 1], padding='SAME')  # 卷积计算
        convResult = tf.nn.bias_add(conv, conv_b)  # 加上偏置项
    return convResult
...
def crlc_model_5_v1(input_tensor):
    Id = 1
    tensor = tf.nn.relu(conv2d(input_tensor, [3, 3, 1, 16], "layer_%02d" % (Id)))
    Id += 1
    tensor = tf.nn.relu(conv2d(tensor, [3, 3, 16, 16], "layer_%02d" % (Id)))
    Id += 1
    tensor = tf.nn.relu(conv2d(tensor, [3, 3, 16, 16], "layer_%02d" % (Id)))
    Id += 1
    tensor = tf.nn.relu(conv2d(tensor, [3, 3, 16, 8], "layer_%02d" % (Id)))
...
def crlc_model_5(input_tensor):
    Id = 1
    tensor = tf.nn.relu(conv2d(input_tensor, [3, 3, 1, 16], "layer_%02d" % (Id)))
    Id += 1
    tensor = tf.nn.relu(conv2d(tensor, [1, 1, 16, 32], "layer_%02d" % (Id)))
    Id += 1
    tensor = tf.nn.relu(conv2d(tensor, [3, 3, 32, 8], "layer_%02d" % (Id)))
    Id += 1
    tensor = tf.nn.relu(conv2d(tensor, [3, 3, 8, 8], "layer_%02d" % (Id)))
    Id += 1
    tensor = conv2d(tensor, [3, 3, 8, 2], "layer_%02d" % (Id))
